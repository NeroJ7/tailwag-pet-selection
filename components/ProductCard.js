...
const ProductCard = ({ product }) => {
  return (
    <Link href={`/product/detail?id=${product.id}`}>
      <div className="premium-card group relative flex flex-col items-center text-center cursor-pointer">
        <div className="relative aspect-[3/4] overflow-hidden rounded-[2.5rem] w-full mb-10 group-hover:shadow-premium transition-all duration-700 hover:shadow-2xl">
          ...
          <div className="absolute bottom-10 left-10 right-10 translate-y-20 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-700 ease-out z-20">
            <div className="w-full bg-white text-brand-charcoal py-5 rounded-full font-black text-[10px] uppercase tracking-[0.4em] shadow-2xl hover:bg-brand-orange hover:text-white transition-all duration-500 hover:scale-105 active:scale-95 text-center">
        ...
        <div className="w-full px-6 flex flex-col items-center">
          ...
          <div className="mb-8 opacity-80 group-hover:opacity-100 transition-opacity duration-700">
            <p className="text-sm text-brand-stone leading-loose font-medium italic">
